...
class ImageScorer:
    ...
    def calculate_clip_score(self, image_path, text) -> float:
        if not text.strip():
        ...
        with torch.no_grad():
            ...
            raw_score = outputs.logits_per_image.cpu().float().numpy()[0][0]
...
def batch_score_videos(base_dir: str,
    ...
    meta = json.load(open('path/to/ReasonVOS/meta_expressions.json'))['videos']
    ...
    for video_dir_name in tqdm(meta.keys()):
        ...
        for expression in meta[video_dir_name]['expressions']:
            # print(f"Processing expression {exp_id}...")
            reference_text = expression['exp_text']
            exp_id = str(expression['exp_id'])
            raw_clip_scores = {}
...
def main():
    ...
    parser.add_argument('--input_dir', default='path/to/ReasonVOS/JPEGImages',
    ...
    parser.add_argument('--output', '-o', default='path/to/video_scores_reasonvos.json',
